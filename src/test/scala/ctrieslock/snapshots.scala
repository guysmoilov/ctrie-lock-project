...
class SnapshotSpec extends WordSpec with Matchers {
  ...
  "A ctrielock" should {
    ...
    "empty 2 quiescent snapshots in isolation" in {
      ...
      class Worker(trie: ConcurrentTrie[Wrap, Int]) extends Thread {
        override def run() {
          for (i <- 0 until sz) {
    ...
    class Modifier(trie: ConcurrentTrie[Wrap, Int], index: Int, rep: Int, sz: Int) extends Thread {
    ...
    class Remover(trie: ConcurrentTrie[Wrap, Int], index: Int, totremovers: Int, sz: Int) extends Thread {
